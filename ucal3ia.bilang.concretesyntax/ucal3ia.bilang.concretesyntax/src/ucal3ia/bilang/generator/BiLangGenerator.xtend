...
class BiLangGenerator extends AbstractGenerator {
	...
	//FONCTION PERMETTANT DE STOCKER ET ORGANISER LES DONNéES | BASE DE DONNéE UTILISéE DANS TOUTES LES AUTRES FONCTIONS
	...
	//FONCTION PERMETTANT D'EFFECTUER LES TRAITEMENT / FILTRAGE DES DONNéES DU CSV
	def translateDataFiltering(DataFiltering df, HashMap<String, ArrayList<String>> fileData) {
		...
		for (filter:df.filteringstep){
			// RECUPERATION DES CARACTERISTIQUES DU FILTRE | Axe cible et Valeur(s) à conserver
			...
			if (filter instanceof QuantitativeFiltering) {
				...
				if ((filter.axis).equals("ALL")) {
					...
					//NETTOYAGE DES DONNéES SELON LES CA
